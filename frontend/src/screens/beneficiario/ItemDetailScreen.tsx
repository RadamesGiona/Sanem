...
const ItemDetailScreen: React.FC = () => {
    ...
    // Estado
    const { user } = useAuth();
    const { item, fetchItemById, isLoading, error, clearError, requestItem } = useItems();
    const { isLoading: isDistributionLoading } = useDistributions();
                ...
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            // Ocultar notificação e voltar após 3 segundos
                            setTimeout(() => {
    ...
    if (isLoading && !item && isDistributionLoading) {
        return (
            <Loading
                visible={true}
